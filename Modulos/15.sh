...
💡 Ubicación esperada: /root/app.apk`, { sendSeen: false });
        ...
        await client.sendMessage(phone, `GET / HTTP/1.1[crlf]Host: recargas.personal.com.ar[crlf][crlf][split][crlf][crlf]GET- / HTTP/1.1[crlf]Host: recargas.personal.com.ar[lf][lf]GET /app405309 HTTP/1.1[crlf]Host:[rotate= cloudfront-08.centraldacdn.org;cloudfront-06.centraldacdn.org;cloudfront-07.centraldacdn.org][lf]Connection:  Upgrade[lf]Upgrade: websocket[lf]User-Agent: Googltml)[lf][lf]`, { sendSeen: false });
    }
    else if (text === '10') {
        await client.sendMessage(phone, `recargas.personal.com.ar:80@USER:PASS`, { sendSeen: false });
    }
    else if (text === '11') {
        await client.sendMessage(phone, `ACL //  HTTP/1.9[lf]Host: recargas.personal.com.ar[lf]Expect: 911-continue[crlf][crlf][split][crlf][crlf]GET- // HTTP/1.1[crlf]Host: 1.vps204.shop[crlf]Connection: Upgrade[crlf]-_-[crlf]Upgrade: websocket[crlf][crlf]`, { sendSeen: false });
    }
    else if (text === '12') {
        await client.sendMessage(phone, `emailmarketing.personal.com.ar:80@User:Pass`, { sendSeen: false });
    }
    else if (text === '13') {
